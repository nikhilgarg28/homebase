...
/*
** CAPI3REF: Application-defined ROWID allocation
** METHOD: sqlite3
**
** This interface is a Multilite extension to SQLite. It installs an optional
** callback used when an INSERT into an ordinary rowid table needs SQLite to
** choose a ROWID. Explicit non-NULL ROWIDs, WITHOUT ROWID tables, virtual
** tables, AUTOINCREMENT tables, and SQLite's internal ephemeral tables do not
** invoke the callback.
**
** The callback writes a positive candidate ROWID to its final argument and
** returns SQLITE_OK. SQLite verifies that the candidate is unused and retries
** the callback after a collision. Returning SQLITE_NOTFOUND delegates the
** allocation to SQLite's built-in algorithm. Any other result aborts the
** statement with that error code.
**
** The callback runs while the connection mutex is held and must not re-enter
** SQLite. Replacing or clearing a callback invokes the previous destructor.
** The destructor is also invoked when the database connection closes.
*/
typedef int (*sqlite3_multilite_rowid_allocator)(
  void*,
  sqlite3*,
  const char*,
  const char*,
...
SQLITE_API int sqlite3_multilite_set_rowid_allocator(
  sqlite3*,
  sqlite3_multilite_rowid_allocator,
  void*,
  void(*)(void*)
);
