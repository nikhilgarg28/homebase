//! The replica — the driver over both contracts: one [`MetaStore`] (durable
//! truth), one [`ServerHandle`] (the seven verbs), one injected
//! [`HybridClock`] (the DST discipline: the replica never reads an
//! ambient clock).
//!
//! # No mirror, single driver
//!
//! [`Replica::open`] loads durable state once — to certify it (the audit
...
//! replica holds no copy of any collection, so there is nothing to drift —
//! just a handful of scalars: the identity, the seq-counter shadow, the
//! queue-scan bound.
//!
//! Methods take `&mut self`: one replica drives one store, transitions are
//! serialized by the borrow checker itself. If a storage transition
//! faults, drop the replica and reopen — the store is the truth.
//!
...
//! high-water is the tripwire: an [`open`](Replica::open) that reads a
...
//! the caller makes. [`acquire`](Replica::acquire) is idempotent: a spec
...
//! # The pusher
//!
//! [`push`](Replica::push) drains the queue FIFO, reading head windows
//! from the store as it goes. Consecutive commits to the same space merge
//! into one wire batch (up to [`with_push_cap`](Replica::with_push_cap)
...
//!   [`ReplicaError::Fork`]. (Re-mint-and-requeue recovery is a later
...
//!   [`discard_from`](Replica::discard_from), the rollback. Lease-plane
...
/// What the replica can fail with. Kernel rejections *inside a push* are
...
pub enum ReplicaError {
    ...
    /// truth; drop the replica and reopen.
    Storage(StorageError),
    /// The space cipher failed to open or transform data.
    Cipher(CipherError),
    /// The nonce source failed while preparing an encrypted value.
    Nonce { reason: String },
...
impl From<CipherError> for ReplicaError {
    fn from(err: CipherError) -> Self {
        Self::Cipher(err)
    }
}

impl From<SpaceError> for ReplicaError {
...
/// What [`Replica::acquire`] hands back: the leases satisfying each spec,
...
pub struct Acquired {
    ...
    /// obligation — [`pull`](Replica::pull) the acquired prefixes to at
...
pub enum PushOutcome {
    ...
    /// ([`Replica::discard_from`]) for a faulty commit, re-acquire and
...
/// A held lease as the replica reads it: the durable record joined with
...
pub struct Replica<M, H, C, N = SystemNonceSource> {
    store: M,
    server: H,
    clock: C,
    nonce_source: N,
...
impl<M: MetaStore, H: ServerHandle, C: HybridClock> Replica<M, H, C, SystemNonceSource> {
    ...
    /// seed; the replica itself never touches an entropy source).
    ///
    ...
    pub async fn open(
        ...
        Self::open_with_nonce_source(store, server, clock, fresh, envelope, SystemNonceSource).await
    }
}

impl<M: MetaStore, H: ServerHandle, C: HybridClock, N: NonceSource> Replica<M, H, C, N> {
    pub async fn open_with_nonce_source(
        store: M,
        server: H,
        clock: C,
        fresh: DeviceId,
    ...
    pub async fn leases(&self, prefixes: &[Key]) -> Result<Vec<LeaseState>, ReplicaError> {
        let now = self.clock.stamp();
        let prefixes = self.encode_keys(prefixes)?;
        let mut out = Vec::new();
        for held in self.store.leases_covering(self.space, &prefixes).await? {
            let live = self.lease_usable_for_held(self.space, &held, &now).await?;
    ...
    pub async fn commit(&mut self, entries: Vec<(Key, Value)>) -> Result<Committed, ReplicaError> {
    ...
    /// lease's own prefix. The lower-level [`Replica::acquire`] remains
    ...
    pub async fn ensure(
        ...
        for lease in &acquired.leases {
            let held = self.held_lease_by_id(self.space, lease.id).await?;
            if held.barrier.is_some()
                && !self
                    .lease_usable_for_held(self.space, &held, &self.clock.stamp())
                    .await?
    ...
    pub async fn renew(&mut self, prefixes: &[Key]) -> Result<RenewResponse, ReplicaError> {
        let prefixes = self.encode_keys(prefixes)?;
        let held = if prefixes.is_empty() {
            Vec::new()
        } else {
            self.store.leases_covering(self.space, &prefixes).await?
        };
        let ids: Vec<LeaseId> = held
            ...
            .map(|h| h.lease.id)
            .collect();
        self.renew_ids(self.space, &ids, &held).await
    }

    /// The renewal replica room: `held` must contain a record for every
    ...
    async fn renew_ids(
        ...
        ids: &[LeaseId],
        held: &[HeldLease],
    ) -> Result<RenewResponse, ReplicaError> {
        if ids.is_empty() {
    ...
    pub async fn release(&mut self, ids: &[LeaseId]) -> Result<(), ReplicaError> {
        if ids.is_empty() {
            return Ok(());
        }
        self.reject_release_if_queued_writes(self.space, ids)
            .await?;
        self.store.retire_leases(self.space, ids).await?;
        self.server
            .release(
                &self.space,
                ReleaseRequest {
            ...
            .await?;
        self.store.drop_leases(self.space, ids).await?;
        Ok(())
    }
    ...
    async fn reject_release_if_queued_writes(
        ...
        for (seq, record) in &state.oplog {
            ...
            for held in &releasing {
                ...
                {
                    return Err(ReplicaError::ReleaseBlocked {
                        lease: held.lease.id,
                        at: *seq,
                    });
                }
    ...
    pub async fn pull(&mut self, range: Range) -> Result<ReadAtResponse, ReplicaError> {
        let range = self.cipher.encode_range(&range)?;
        self.pull_encoded(range).await
    }

    async fn pull_encoded(&mut self, range: Range) -> Result<ReadAtResponse, ReplicaError> {
        let space = self.space;
    ...
    /// ([`ReplicaError::Unavailable`], queue intact), or a fork is proven
    /// ([`ReplicaError::Fork`]). See the module docs for the recovery
    /// algebra.
    pub async fn push(&mut self) -> Result<PushOutcome, ReplicaError> {
        let mut acked = None;
        ...
        loop {
            ...
            match self.server.put_batch(&space, request).await {
                ...
                Err(SpaceError::Kernel(KernelError::DeviceSeqRegression { current, .. })) => {
                    ...
                    if !ours {
                        return Err(ReplicaError::Fork { admitted: current });
                    }
                ...
                Err(SpaceError::Unavailable { reason }) => {
                    return Err(ReplicaError::Unavailable { reason });
                }
            }
        }
    }
    ...
    pub async fn discard_from(&mut self, from: DeviceSeq) -> Result<(), ReplicaError> {
    ...
    /// bound.
    async fn ack(&mut self, through: DeviceSeq) -> Result<(), ReplicaError> {
    ...
    async fn check_local_write_authority(
        ...
        for key in keys {
            ...
            if !covered {
                return Err(ReplicaError::LocalAuthority { key });
            }
        }
        Ok(())
    }

    fn encode_keys(&self, keys: &[Key]) -> Result<Vec<Key>, ReplicaError> {
        keys.iter()
            .map(|key| Ok(self.cipher.encode_key(key)?))
            .collect()
    }

    fn encode_specs(&self, specs: Vec<LeaseSpec>) -> Result<Vec<LeaseSpec>, ReplicaError> {
        specs
            .into_iter()
            .map(|spec| {
                Ok(LeaseSpec {
    ...
    async fn usable_covering<'a>(
        ...
        spec: &LeaseSpec,
        now: &HybridTimestamp,
    ) -> Result<Option<&'a Lease>, ReplicaError> {
        for h in held {
    ...
    async fn max_pending_barrier(
        ...
        specs: &[LeaseSpec],
        now: &HybridTimestamp,
    ) -> Result<Option<AdmissionSeq>, ReplicaError> {
        let mut out = None;
        for spec in specs {
            for held in held {
                if held.retiring
    ...
    async fn live_write_leases(
        ...
        space: SpaceId,
        keys: &[Key],
    ) -> Result<Vec<LeaseRef>, ReplicaError> {
        let now = self.clock.stamp();
        let mut out = Vec::new();
