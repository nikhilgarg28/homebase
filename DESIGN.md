...
**Deployment shape:** a server owns **shards** of spaces; shard = one slatedb instance (object-store-native LSM: local-NVMe object store for dev/self-host, S3 for prod) = one WAL, group-committing across its spaces (storage key = space id ⊕ kind ⊕ encoded key; per-space records: data with inline tags, changelog keyed by latest-change admission seq, lease by-id + by-prefix indexes, counters). Leases live in slatedb like everything else: grant + counters commit in one atomic write batch, so epoch monotonicity and the lease table survive crashes exactly (recovery = WAL replay, nothing special-cased). Cost: acquire waits on a WAL flush — fine on NVMe, tens of ms on S3; revisit with async-durable lease writes + restart grace ≥ max TTL if acquire latency ever demands it.
...
Wire protocol (lean tonic) · changeset format custom vs. sessions (lean custom) · rung 4.5 in/out · repo public-from-spec vs. private-until-rung-4 · multi-process lease broker (post-v1) · space→shard placement/rebalancing (post-v1; slatedb manifest views may give O(1) key-range splits) · homebase clearance (homestead needs none) · key limits (16 comps × 256B proposed) · read-tier API naming · LWW table class (deferred; would layer above kernel, HLC server-side max-wins if ever built).