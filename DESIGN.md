...
One ordered map per space — tuple keys (`Vec<Bytes>`, component-wise prefix, FDB-style order-preserving flat encoding) → opaque values, tagged `(device, device_seq, epoch, ver, admission_seq)` — plus a lease table, registered subscriptions, and an **augmented tree maintaining max-admission-seq under any prefix**. Eight verbs:

```
acquire/renew/release   # X-only. TTL, max_puts one-shots, batch acquire → barrier seq,
                        # contention piggybacks on renewal, steal denied pre-deadline
put_batch               # atomic; every key covered by valid lease; epoch-fenced;
                        # per-key ver monotonicity enforced; tombstone deletes
get / list
sync(prefixes, cursors)              # delta fetch; drives shapes
read_at(ranges, cursors) -> (S, Δ)   # atomic consistent cut at one admission point
```

Prefix-scoped tokens enforced on reads AND writes. **X-only** (S mode deleted: FK reader-writer semantics reconstruct from exclusivity + prefix hierarchy — child inserts take X on per-child guard keys or serialize on the parent row; ledger appends = long X on device-scoped prefixes). Invariants: no write without covering valid lease · no overlapping leases, no upgrades, ever · epochs are correctness, timestamps availability (asymmetric expiry) · strict local expiry · barrier = serializability · demand-driven stickiness, min-hold ≈ 2× heartbeat · TTL policy: kernel cap → class defaults (witness ~10s non-sticky, row ~5m, db ~1h) → app pin.

**Honest-but-dumb:** SIV per-component key pseudonymization (prefix relations survive at component boundaries); XChaCha20-Poly1305 values with coordination tags bound in the AEAD associated data (splice/replay-proof); client-computed per-key ver chains (server enforces monotonicity); subtree anchors at generation boundaries for fork detection (full fork-consistency explicitly out of scope). Server clock exists in one code path (lease deadlines); server trust in clients: zero.
...
**Witness compiler (the novel contribution):** speculative execute in savepoint → concrete write set via preupdate hook (catches triggers/cascades) → derive leases (row X; uniqueness witness on collated value; FK guard) → sorted batch-acquire (no deadlocks) → barrier → re-execute → coverage check → retry budget → table escalation. Handles collations, expression/partial indexes, read-beyond-row triggers (static scan → escalate), rowids (per-device blocks via one-shot). Query-side twin compiles reads to range fetches (PK/index point-gets, FK closure; unindexed → subscribe-table fallback). Compat: swap the open call; same dialect (it *is* SQLite); `SQLITE_BUSY` for contention/offline; DDL requires db-level X lease.
...
Each is a thin client library over the same eight verbs — no kernel changes:

- **Job queue (SQS-lite)** — one-shot lease (`max_puts: Some(1)`) = claim; TTL = visibility timeout; crash = reclaim; epoch = zombie-worker fencing. *Adopted as rung 2.5 dogfood consumer: a few hundred lines, no SQLite, exercises one-shots/TTL/contention/fencing under crash injection before the compiler exists.*
- **Distributed cron** — one-shot on `("cron", job, slot)` = exactly-once firing across N schedulers.
- **Leader election / service registry** — long X lease = leadership; epoch = the fencing token handed downstream; the Chubby use cases, E2EE-compatible.
- **Kafka-lite** — admission seq = total order; cursors = consumer offsets; consumer groups = one-shot claims on partition prefixes. Encrypted event streaming at team scale.
- **Config / feature-flag distribution** — admin holds X lease, fleets poll `read_at`: consistent multi-key config cuts, never torn flag combinations; range seq = free config version.
...
- **CRDT sync backend** — per-device X prefixes carrying Automerge/Yjs ops; drop-in encrypted relay for the existing local-first ecosystem. Adoption wedge.
...
1. **Kernel, in-memory** — client + server crates, all eight verbs, tuple encoding, augmented tree. *Exit: proptest invariant suite (overlap, fencing, barrier, expiry asymmetry, ver monotonicity, read_at cut correctness). The crown jewel.*
