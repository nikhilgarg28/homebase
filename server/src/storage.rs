//! The ordered-map storage abstraction the kernel is built against.
//!
//! The kernel's state machine reads and writes a single ordered byte-keyed
//! map per shard through [`OrderedStore`]. The trait is async and fallible
//! because the production implementation (slatedb: local-NVMe object store
//! for dev, S3 for prod) does real IO that can fail. [`MemoryStore`] backs
//! tests and the deterministic sim — its futures resolve immediately and
//! never fail, so determinism is preserved: verbs execute serially, and no
//! await point inside a verb can interleave with another.
//!
...
/// [`KernelError`](homebase_core::messages::KernelError): kernel errors are
