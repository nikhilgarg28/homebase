...
- Optimize `Space::analyze_rebase`. The first implementation scans the selected admitted mutations once per active range assertion (`O(assertions * mutations)`) and materializes both windows. Build a bounded-memory prefix index or streaming equivalent that answers descendant point/range and ancestor DeleteRange maxima while preserving assertion order, exact `actual` sequences, and the own-admission cap.
- Define reusable interval bookkeeping above Homebase rebase analysis. Homebase deliberately analyzes only the caller-selected retained `[from, to)` admit range; Multilite must atomically verify the returned submit/admit cursor snapshots, reject gaps or stale ranges, apply the same interval, and advance its reconciliation frontier without treating Homebase `mark_applied` as implicit conflict analysis.
