...
//!
//! - `HOMESTEAD_TORTURE_SEEDS` — crash / steal / replica / persist (default 1000)
//! - `HOMESTEAD_SCENARIO_SEEDS` — scenario harness (default 100)
...
pub fn torture_seed_count() -> u64 {
    parse_env("HOMESTEAD_TORTURE_SEEDS").unwrap_or(DEFAULT_TORTURE)
}

/// Layer 3 (slatedb persist) seeds — separate knob; slatedb is much slower.
pub fn persist_torture_seed_count() -> u64 {
    parse_env("HOMESTEAD_PERSIST_TORTURE_SEEDS").unwrap_or(DEFAULT_PERSIST)
}

/// Number of seeds per scenario in `tests/scenarios.rs`.
pub fn scenario_seed_count() -> u64 {
    parse_env("HOMESTEAD_SCENARIO_SEEDS").unwrap_or(DEFAULT_SCENARIO)
}
